...
class KalmanFilterXYAH:
    ...
    def __init__(self):
        ...
        self._update_mat = np.eye(ndim, 2 * ndim) # 状态转移矩阵F
    ...
    def initiate(self, measurement):
        ...
        mean = np.r_[mean_pos, mean_vel] # 初始化状态估计值x，
        ...
        covariance = np.diag(np.square(std)) # 初始化先验估计协方差
        return mean, covariance # 初始化状态估计值x， 初始化先验估计协方差P
    ...
    def project(self, mean, covariance):
        ...
        covariance = np.linalg.multi_dot((self._update_mat, covariance, self._update_mat.T))  # 主要根据目标的高度构造x的协方差矩阵
    ...
    def multi_predict(self, mean, covariance):
        ...
        motion_cov = [np.diag(sqr[i]) for i in range(len(mean))]    # 初始化噪音Q
        motion_cov = np.asarray(motion_cov)

        mean = np.dot(mean, self._motion_mat.T)  # 更新状态估计值x
        left = np.dot(self._motion_mat, covariance).transpose((1, 0, 2))
        covariance = np.dot(left, self._motion_mat.T) + motion_cov  # 更新先验估计协方差P

        return mean, covariance  # 更新状态估计值x， 更新先验估计协方差P
    ...
    def update(self, mean, covariance, measurement):
        ...
        """
        # 利用上一时间的后验估计计算x=Fx和先验估计的协方差P=FPF+Q
